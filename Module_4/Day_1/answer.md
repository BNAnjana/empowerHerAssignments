...
Displays visual elements such as buttons, forms, menus, images, and text

Focuses on layout, design, responsiveness, and accessibility
...
Provides instant feedback (e.g., form validation, animations, error messages)

Enhances user experience through smooth and interactive behavior

3. Communication with Backend

Sends requests to the backend using HTTP/HTTPS (GET, POST, PUT, DELETE)

Receives responses (usually JSON) from APIs

Displays backend data dynamically on the UI
...
It sits between the frontend and database and determines what should happen when a user performs an action.
